  ...
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.4) {
        entry.target.classList.add("bank-show");
      }
      // else {
  ...
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        entry.target.play(); // Play the video when it intersects with the viewport
      } else {
        entry.target.pause(); // Pause the video when it's not visible
      }
    });
  },
  { threshold: 0.6 }
...
tl.fromTo(".cta1", { y: "100%", opacity: 0.5 }, { y: 0, opacity: 1 }, "<20%");
tl.fromTo(".cta2", { y: "100%", opacity: 0.5 }, { y: 0, opacity: 1 }, "<20%");
tl.fromTo(".cta3", { y: "100%", opacity: 0.5 }, { y: 0, opacity: 1 }, "<20%");
tl.fromTo(".cta4", { y: "100%", opacity: 0.5 }, { y: 0, opacity: 1 }, "<20%");
